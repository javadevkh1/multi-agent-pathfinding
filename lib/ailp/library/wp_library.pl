...
% Issue query Q to Wikipedia and return the persons persondata
...
get_persondata([X|Xs],Out) :-
  ...
  ; otherwise -> Out = Ys  % skip X if it doesnt contain =
  ),
...
% version of atomic_list_concat/3 that fails if separator doesnt occur
...
test :-
  init_identity(_A),  % initialise; dont reveal identity
