...
% Agents position needs to be map_adj to charging station identified by OID
internal_topup_energy(Agent, OID) :-
  nonvar(Agent),
  nonvar(OID),
  agent_current_position(Agent,Pos),
...
% Agents position needs to be map_adj to oracle identified by OID
...
% Has constraint that To is map_adj to Agents current position
