...
% First tries to find cw_*.pl files (academic submission format)
% Falls back to src/ folder files (portfolio format)
find_submission(Assignment, Submission) :-
  ...
  ; Assignment='oscar'       -> 
      ( find_file('cw_', Files, Submission) 
      ; exists_file('src/pathfinding.pl'), Submission = 'src/pathfinding.pl'
      )
  ; Assignment='wp'          -> 
      ( find_file('cw_wp_', Files, Submission)
      ; exists_file('src/route_planner.pl'), Submission = 'src/route_planner.pl'
      )
  ; Assignment='maze'        -> 
      ( find_file('cw_maze_',Files, Submission)
      ; exists_file('src/multi_agent_coordinator.pl'), Submission = 'src/multi_agent_coordinator.pl'
      )
  ).
...
get_assignment_details :- 
    ...
    % If LibDir ends with '/lib', remove it to get project root
    ( atom_concat(ProjectRoot, '/lib', LibDir) -> true
    ; atom_concat(ProjectRoot, '\\lib', LibDir) -> true  % Windows path
    ; ProjectRoot = LibDir  % Fallback if not in lib/ subdirectory
    ),
    % define a module path assignment_root(./) - points to project root
    (\+ user:file_search_path(assignment_root, ProjectRoot)
    ->  asserta(user:file_search_path(assignment_root, ProjectRoot))
    ),
    
    % define our own ailp path assignment_ailp(./lib/ailp)
    atom_concat(ProjectRoot, '/lib/ailp', Ailp),
    (\+ user:file_search_path(assignment_ailp, Ailp)
    ->  asserta(user:file_search_path(assignment_ailp, Ailp))
    ),
    
    % define our own Library path assignment_library(./lib/ailp/library)
